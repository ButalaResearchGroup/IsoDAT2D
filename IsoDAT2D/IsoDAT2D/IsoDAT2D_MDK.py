...
def run_nimfa_nmf_new(data, n_components):
       # Convert DataFrame to numpy array if needed
    if isinstance(data, pd.DataFrame):
        data = data.values    

    # Use sklearn's NMF implementation instead of nimfa
    from sklearn.decomposition import NMF
    nmf = NMF(n_components=n_components, init='random', max_iter=600, random_state=42)
    W = nmf.fit_transform(data)
    H = nmf.components_
    err = nmf.reconstruction_err_
    
    print('Reconstruction error: %5.3f' % err)
    print('Number of iterations: %d' % nmf.n_iter_)
...
def run_nmf_and_agg_cluster(rotated_data, n_components, n_clusters):
    ...
    basis, coefficient, err = run_nimfa_nmf_new(rotated_data, n_components)
    ...
    plt.scatter(n_components, err, marker='o')
...
def run_semisupervised_nmf(data, labels, max_components=2, lambda_penalty=10.0, max_iter=600, tol=1e-4, init='random', solver='cd', random_state=None):
    """
    Semi-supervised NMF: minimize ||V - W H||^2 + lambda * ||H - Y||^2
    where Y is a one-hot encoding of the known labels for each sample.

    Parameters:
    - data:            (n_samples, n_features) data matrix V^T
    - labels:          array of length n_samples with integer class labels 0..(k-1)
    - max_components:  number of latent components (should match number of classes or more)
    - lambda_penalty:  weight on the label-fitting term
    - max_iter:        maximum number of update iterations
    - tol:             convergence tolerance on objective change
    - init:            initialization method passed to sklearn NMF
    - solver:          solver passed to sklearn NMF ('cd' or 'mu')
    - random_state:    random seed

    Returns:
    - W:               basis matrix (n_samples, n_components)
    - H:               coefficient matrix (n_components, n_features)
    - errors:          list of objective values per iteration

    """
   
    # Ensure labels are integer-coded from 0 to k-1
    unique_labels = np.unique(labels)
    label_to_idx = {lab: idx for idx, lab in enumerate(unique_labels)}
    labels_int = np.array([label_to_idx[lab] for lab in labels], dtype=int)
    n_labels = len(unique_labels)
    if max_components < n_labels:
        raise ValueError(f"max_components ({max_components}) must be >= number of unique labels ({n_labels})")

    print("data.shape[0] =", data.shape[0])
    print("len(labels)   =", len(labels))
    assert data.shape[0] == len(labels), "number of labels must equal number of samples"

    # Prepare V (features × samples) and label matrix Y (components × samples)
    V = data.T  # shape (n_features, n_samples)
    n_features, n_samples = V.shape
    n_components = max_components

    # Build one-hot Y
    Y = np.zeros((n_components, n_samples))
    for i, lab_idx in enumerate(labels_int):
        Y[lab_idx, i] = 1

    # Initialize W and H via one-step NMF
    base_model = NMF(n_components=n_components, init=init, solver=solver,
                     tol=tol, max_iter=1, random_state=random_state)
    W = np.abs(base_model.fit_transform(data)).T  # now shape (n_components, n_features)
    H = np.abs(base_model.components_)           # shape (n_components, n_features)
    ...
    for it in range(max_iter):
        ...
        # Update H with label penalty
        numerator_H = W.T @ V + lambda_penalty * Y
        denominator_H = (W.T @ W) @ H + lambda_penalty * H + 1e-9
        H *= numerator_H / denominator_H

        # Compute objective
        recon = V - W @ H
        obj = np.linalg.norm(recon, 'fro')**2 + lambda_penalty * np.linalg.norm(H - Y, 'fro')**2
        errors.append(obj)

        if prev_obj is not None and abs(prev_obj - obj) < tol:
            logging.info(f'Converged at iteration {it}, objective change {abs(prev_obj - obj):.2e}')
            break
        prev_obj = obj

    # Transpose W back to (n_samples, n_components)
    W_out = W.T
    H_out = H
    ...
    plt.title('Semi-supervised NMF Objective')
    plt.xlabel('Iteration')
    plt.ylabel('Objective value')
    plt.grid(True)
    plt.show()

    return W_out, H_out, errors
...
def run_sklearn_nmf_and_hdbscan(data, max_components, max_iter=600, init='random', solver='cd', tol=1e-4, patience=5, min_cluster_size = 10, min_samples = 5, cluster_matrix = 'W'):
    ...
    best_W, best_H, best_reconstruction_err = run_sklearn_nmf(data, max_components, max_iter, init, solver, tol, patience)
    if cluster_matrix == 'W':
        data_dict = cluster_results_basis_hdbscan(best_W, min_cluster_size , min_samples)
    else:
        data_dict = cluster_results_weights_hdbscan(best_W, best_H, min_cluster_size , min_samples)
